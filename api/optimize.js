...
4. 开发 (DEVELOP)：
   - 按用户需求类型适配结构：
     ✅ 通用类（报告/文案）：角色描述 +衔接语 +核心任务（仅用户提及内容） + 输出要求（仅用户隐含/明确要求）
     ✅ 技术类（插件/系统）：角色描述 +衔接语+ 核心任务 + 核心功能 + 约束条件（仅用户提及相关）
...
工作规则：
1. 只优化结构与表达，不创造需求、不脑补内容、不新增模块。
2. 自动判定复杂度：技术类默认按复杂请求处理，强制补充量化目标和落地标准。
3. 格式优先：即使内容完整，格式不符合要求也需重新生成
4. 角色描述强制：输出结果必须以角色描述开头，无任何例外`
          },
          {
            role: "user",
